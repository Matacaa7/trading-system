...
def load_models(modelos_cfg: list[dict]) -> list[dict]:
    ...
    for m_cfg in modelos_activos:
        ...
        try:
            ...
            # ── Cargar modelo con la clase wrapper correcta ─────────
            model = _load_model(model_name, file_path)
            if model is None:
                continue
...
def _load_model(model_name: str, file_path: Path):
    """Carga un modelo usando la clase wrapper correcta del monorepo."""
    try:
        if model_name == "xgboost":
            from shared.models.sklearn_models.xgboost_model import XGBoostModel
            return XGBoostModel.load(file_path, task="classification")
        elif model_name == "random_forest":
            from shared.models.sklearn_models.random_forest_model import RandomForestModel
            return RandomForestModel.load(file_path, task="classification")
        elif model_name == "lightgbm":
            from shared.models.sklearn_models.lightgbm_model import LightGBMModel
            return LightGBMModel.load(file_path, task="classification")
        elif model_name == "lstm":
            from shared.models.pytorch_models.lstm_model import LSTMModel
            return LSTMModel.load(file_path, task="classification")
        elif model_name == "gru":
            from shared.models.pytorch_models.gru_model import GRUModel
            return GRUModel.load(file_path, task="classification")
        elif model_name == "transformer":
            from shared.models.pytorch_models.transformer_model import TransformerModel
            return TransformerModel.load(file_path, task="classification")
        else:
            log.warning(f"  Modelo no reconocido: {model_name}")
            return None
    except Exception as e:
        log.error(f"  Error instanciando {model_name} desde {file_path}: {e}")
        return None
