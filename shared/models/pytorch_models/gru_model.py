...
gru_model.py
────────────
Implementación de GRU en la interfaz BaseModel.
Similar a LSTM pero más ligero y rápido de entrenar.
...
class GRUModel(BaseModel):
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray) -> None:
        ...
        for epoch in range(self.epochs):
            ...
            if (epoch + 1) % 10 == 0:
                log.info(
                    f"  GRU Epoch {epoch+1}/{self.epochs} "
    ...
    def _get_sequences(self, X: np.ndarray) -> torch.Tensor:
        ...
        if len(X) < self.seq_len:
            ...
            log.warning(
                f"GRU: secuencia corta ({len(X)} < {self.seq_len}), "
        ...
        torch.save(
            {
                ...
                "input_size": self._net.gru.input_size,
                "hidden_size": self.hidden_size,
                "num_layers": self.num_layers,
                "dropout": self.dropout,
                "task": self.task,
